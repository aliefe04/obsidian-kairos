...
| `channels/{desktop,ntfy,ics}.ts` | The shipped channels: desktop notifications, an ntfy push and an in-vault calendar file |
| `channels/ical.ts` | The iCalendar primitives the calendar file is written with, so there is one folding rule, not two |
